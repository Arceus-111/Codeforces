...
// place only one tile (drain) 
// if i fix a column , its always better to go deep
// i can have bi = cnt of water 
// bi = h - ai 
// for any bi and bj 
// whatever cell we choose , we always get q
// 
void solve()
{
    //your code
    ...
    ll ans  = 0LL;
    REP1(i,n){
        ll res = b[i][0] + b[i][n-1] -(h-a[i]) ;
        chmax(ans,res);
    }
    REP1(i,n){
        REP2(j,i+1,n){
            ll res = b[i][0] + b[j][n-1] ;
            ll ex1 = (j-1 >= 0)?b[i][j-1]-(h-a[i]):0;
            ll ex2 = (i+1>=j)?0:b[j][i+1]-(h-a[j]);

            chmax(ans,res + max(ex1,ex2));
            ll idx = c[i][j] ;
            ex1 = b[i][idx]-(h-a[i]);
            ex2 = b[j][idx]-(h-a[j]);
            chmax(ans,res + (ex1+ex2)-(h-a[idx]));
        }
    }
    cout<<ans<<endl;
}
